...
<body>
	...
	<script>
			...
			layui.use(['paging', 'form','laydate'], function() {
                ...
                paging.init({
                    openWait: true,
                    url: 'getSchoolList?name=${name}', //地址
					elem: '#content', //内容容器
					params: { //发送到服务端的参数
					},
					type: 'GET',
					...
					complate: function() { //完成的回调
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=del]').on('click', function() {
								...
								layer.confirm('您确认删除吗?', {icon: 3, title:'提示'}, function(index){
									  //do something
									  $.get("deleteSchool",{id:id},function(data){
									//返回结果并刷新页面
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
								...
								$.get('school-edit', {id:id}, function(form) {
				...
				$('#getSelected').on('click', function() {
					...
					layer.confirm('您确认删除吗?', {icon: 3, title:'提示'}, function(index){
						  //do something
						  $.get("deleteAllSchool",{ids:ids},function(data){
						//返回结果并刷新页面
						   layerTips.msg(data.msg,{time:500},function(){
							   layerTips.close(index);
								location.reload(); //刷新
				...
				$('#add').on('click', function() {
					if(addBoxIndex !== -1)
						return;
					//本表单通过ajax加载 --以模板的形式，当然你也可以直接写在页面上读取
					$.get('school-add', null, function(form) {
						addBoxIndex = layer.open({
							type: 1,
							title: '添加信息',
							content: form,
