...
<td>{{ item.writtenexamination }}</td>
<td>{{ item.interviewexamination }}</td>
<td>{{ item.enclosure1 }}</td>     
<td>{{ item.enclosure2 }}</td>   
<td>{{ item.soundrecording }}</td>   
<td>{{ item.interviewfeedback }}</td>        
	...
	<script>
			...
			layui.use(['paging', 'form','laydate', 'upload'], function() {
                ...
                paging.init({
                    openWait: true,
                    url: 'getInterviewSituationList?name=${name}', //地址
					elem: '#content', //内容容器
					params: { //发送到服务端的参数
					},
					type: 'GET',
				...
				$('#add').on('click', function() {
					if(addBoxIndex !== -1)
						return;
					//本表单通过ajax加载 --以模板的形式，当然你也可以直接写在页面上读取
					$.get('InterviewSituation-add', null, function(form) {
						addBoxIndex = layer.open({
							type: 1,
							title: '添加面试情况',
							content: form,
