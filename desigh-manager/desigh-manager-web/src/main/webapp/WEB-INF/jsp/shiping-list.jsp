...
<body>
	<div class="admin-main">
		...
		<blockquote class="layui-elem-quote">
			<a href="javascript:;" class="layui-btn layui-btn-small" id="add">
				<i class="layui-icon">&#xe608;</i> 添加教学视频信息
			</a> <a href="#" class="layui-btn layui-btn-small" id="getSelected">
				<i class="fa fa-shopping-cart" aria-hidden="true"></i> 删除所选信息
			</a> <a href="shiping-list" class="layui-btn layui-btn-small" id="fluse">
				<i class="layui-icon">&#x1002;</i> 刷新
			</a>
			<form class="layui-form" style="float: right;">
				<div class="layui-form-item" style="margin: 0;">
					<div class="layui-input-inline">
						<input type="text" name="name" placeholder="请输入文件名.."
	...
	<script type="text/html" id="tpl">
			...
			<tr>
				...
				<td>{{ item.title }}</td>						
				<td><a href="{{ item.fn }}" target="_blank"  class="layui-btn layui-btn-mini">下载</a></td>
	...
	<script>
			...
			layui.use(['paging', 'form', 'laydate', 'upload'], function() {
                ...
                paging.init({
                    openWait: true,
                    url: 'getShipingList?name=${name}', //地址
					elem: '#content', //内容容器
					params: { //发送到服务端的参数
					},
					type: 'GET',
					...
					complate: function() { //完成的回调
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=del]').on('click', function() {
								...
								layer.confirm('您确认删除吗?', {icon: 3, title:'提示'}, function(index){
									  //do something
									  $.get("deleteShiping",{id:id},function(data){
									//返回结果并刷新页面
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
								...
								$.get('shiping-edit', {id:id}, function(form) {
				...
				$('#getSelected').on('click', function() {
					...
					layer.confirm('您确认删除吗?', {icon: 3, title:'提示'}, function(index){
						  //do something
						  $.get("deleteAllShiping",{ids:ids},function(data){
						//返回结果并刷新页面
						   layerTips.msg(data.msg,{time:500},function(){
							   layerTips.close(index);
								location.reload(); //刷新
				...
				$('#add').on('click', function() {
					if(addBoxIndex !== -1)
						return;
					//本表单通过ajax加载 --以模板的形式，当然你也可以直接写在页面上读取
					$.get('shiping-add', null, function(form) {
						addBoxIndex = layer.open({
							type: 1,
							title: '添加反馈',
							content: form,
