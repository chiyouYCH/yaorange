		...
		<blockquote class="layui-elem-quote">
			...
			<form class="layui-form" style="float: right;">
				<div class="layui-form-item" style="margin: 0;">
					...
					<div class="layui-input-inline">
						<input type="text" name="stemName" placeholder="请输入题干名.."
							autocomplete="off" class="layui-input">
					</div>
	...
	<script>
			...
			layui.use(['paging', 'form','laydate'], function() {
				...
				$('#search').on('click', function() {
					var $this = $(this);
					var name = $this.prev('input[name=stemName]').val();
					var knowpointName = $this.prev('input[name=knowpointName]').val();
					var courseName = $this.prev('input[name=courseName]').val();
					console.log("name="+name);
					console.log("knowpointName="+knowpointName);
					...
					paging.get({
						t: Math.random(),
						stemName: name ,//获取输入的关键字。
						knowpointName: knowpointName ,
						courseName: courseName
					});
				});
                paging.init({
                    openWait: true,
                    url: 'getTopicList?stemName=${stemName}&knowpointName=${knowpointName}&courseName=${courseName}', //地址
					elem: '#content', //内容容器
					params: { //发送到服务端的参数
					},
					type: 'POST',
					...
					complate: function() { //完成的回调
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=del]').on('click', function() {
								...
								layer.confirm('您确认删除吗?', {icon: 3, title:'提示'}, function(index){
									  //do something
									  $.get("deleteTopic",{id:id},function(data){
									//返回结果并刷新页面
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
								...
								$.get('Topic-edit', {id:id}, function(form) {
				...
				$('#add').on('click', function() {
					if(addBoxIndex !== -1)
						return;
					//本表单通过ajax加载 --以模板的形式，当然你也可以直接写在页面上读取
					$.get('topic-add', null, function(form) {
						addBoxIndex = layer.open({
							type: 1,
							title: '添加面试题',
							content: form,
