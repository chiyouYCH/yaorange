...
<td>{{ item.contactsphone }}</td>          
				<td>{{ null==item.jointime?'':layui.laytpl.toDateString(item.creartetime,'yyyy-MM-dd') }}</td>
<td>{{ item.applicablewages }}</td> 
<td>{{ item.formalwages }}</td> 
<td>{{ item.welfare }}</td> 
<td>{{ item.offer }}</td> 
<td>{{ item.remark }}</td>   
				<td>
	...
	<script>
			...
			layui.use(['paging', 'form','laydate','upload'], function() {
                ...
                paging.init({
                    openWait: true,
                    url: 'getEmploymentSituationList?name=${name}', //地址
					elem: '#content', //内容容器
					params: { //发送到服务端的参数
					},
					type: 'GET',
